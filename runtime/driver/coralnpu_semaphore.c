...
typedef struct iree_hal_coralnpu_semaphore_t {
...
iree_status_t iree_hal_coralnpu_semaphore_create(
  ...
  *out_semaphore = NULL;
  ...
  iree_hal_coralnpu_semaphore_t *semaphore = NULL;
  iree_status_t status = iree_allocator_malloc(
      host_allocator, sizeof(*semaphore), (void **)&semaphore);
  if (iree_status_is_ok(status)) {
    iree_hal_semaphore_initialize(&iree_hal_coralnpu_semaphore_vtable,
                                  &semaphore->base);
...
static iree_status_t iree_hal_coralnpu_semaphore_signal_unsafe(
    iree_hal_coralnpu_semaphore_t *semaphore, uint64_t new_value) {
...
static iree_status_t iree_hal_coralnpu_semaphore_signal(
  ...
  iree_status_t status =
      iree_hal_coralnpu_semaphore_signal_unsafe(semaphore, new_value);
  if (!iree_status_is_ok(status)) {
    iree_slim_mutex_unlock(&semaphore->mutex);
    return status;
  }
...
iree_status_t iree_hal_coralnpu_semaphore_multi_signal(
  ...
  for (iree_host_size_t i = 0; i < semaphore_list.count; ++i) {
    iree_hal_coralnpu_semaphore_t *semaphore =
        iree_hal_coralnpu_semaphore_cast(semaphore_list.semaphores[i]);

    iree_slim_mutex_lock(&semaphore->mutex);
    status = iree_hal_coralnpu_semaphore_signal_unsafe(
...
typedef struct iree_hal_coralnpu_semaphore_notify_state_t {
  iree_hal_coralnpu_semaphore_t *semaphore;
  uint64_t value;
} iree_hal_coralnpu_semaphore_notify_state_t;

static bool iree_hal_coralnpu_semaphore_is_signaled(
    iree_hal_coralnpu_semaphore_notify_state_t *state) {
  iree_hal_coralnpu_semaphore_t *semaphore = state->semaphore;
  iree_slim_mutex_lock(&semaphore->mutex);
...
static iree_status_t iree_hal_coralnpu_semaphore_wait(
  ...
  iree_hal_coralnpu_semaphore_state_t *shared_state = semaphore->shared_state;
  iree_hal_coralnpu_semaphore_notify_state_t notify_state = {
      .semaphore = semaphore,
      .value = value,
  };
...
static iree_status_t iree_hal_coralnpu_semaphore_result_from_state(
...
iree_status_t iree_hal_coralnpu_semaphore_multi_wait(
  ...
  if (iree_timeout_is_immediate(timeout)) {
    iree_status_t status = iree_hal_coralnpu_semaphore_result_from_state(
        wait_mode, semaphore_list);
    IREE_TRACE_ZONE_END(z0);
    return status;
  }
  ...
  iree_status_t status =
      iree_hal_coralnpu_semaphore_result_from_state(wait_mode, semaphore_list);

  IREE_TRACE_ZONE_END(z0);
  return status;
}

static iree_status_t iree_hal_coralnpu_semaphore_import_timepoint(
    iree_hal_semaphore_t *base_semaphore, uint64_t value,
...
static iree_status_t iree_hal_coralnpu_semaphore_export_timepoint(
    iree_hal_semaphore_t *base_semaphore, uint64_t value,
    iree_hal_queue_affinity_t queue_affinity,
...
static const iree_hal_semaphore_vtable_t iree_hal_coralnpu_semaphore_vtable = {
    .destroy = iree_hal_coralnpu_semaphore_destroy,
    .query = iree_hal_coralnpu_semaphore_query,
    .signal = iree_hal_coralnpu_semaphore_signal,
    .fail = iree_hal_coralnpu_semaphore_fail,
    .wait = iree_hal_coralnpu_semaphore_wait,
    .import_timepoint = iree_hal_coralnpu_semaphore_import_timepoint,
    .export_timepoint = iree_hal_coralnpu_semaphore_export_timepoint,
};
